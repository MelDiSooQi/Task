...
function MdButtonDirective($mdButtonInkRipple, $mdTheming, $mdAria, $timeout) {
  ...
  function postLink(scope, element, attr) {
    ...
    element[0].onmspointerdown = function(e){
        if (attr.disabled === true) {
            e.preventDefault();
            e.stopImmediatePropagation();
          }
    ...
    element[0].onmspointerup = function(e){
        if (attr.disabled === true) {
            e.preventDefault();
            e.stopImmediatePropagation();
          }
...
function MdCheckboxDirective(inputDirective, $mdAria, $mdConstant, $mdTheming, $mdUtil, $timeout) {
  ...
  function compile (tElement, tAttrs) {
    ...
    //  }
    //});
    tElement[0].ontouchend = function (event) {
        if (this.hasAttribute('disabled')) {
            event.stopImmediatePropagation();
        }
    };

    tElement[0].onmspointerup = function (event) {
        if (this.hasAttribute('disabled')) {
            event.stopImmediatePropagation();
        }
    };
...
function mdRadioButtonDirective($mdAria, $mdUtil, $mdTheming) {
  ...
  function link(scope, element, attr, rgCtrl) {
    ...
    function initialize(controller) {
      ...
      //element
      //  .on('click', listener)
        element[0].ontouchend = listener;
        element[0].onmspointerup = listener;
